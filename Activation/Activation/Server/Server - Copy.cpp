...
int main()
{
	...
	char		versionNum[STRLEN];
	char        invalidStatus[STRLEN] = "no";
	char		checkVersion[STRLEN];
	...
	while (!done)
	{
		...
		{
			strncpy_s(versionNum, recvMessage, sizeof(versionNum));

			// Send whether the serial number was valid or invalid to the client
			int version = send(acceptSocket, validStatus, strlen(validStatus), 0);

			if (version == SOCKET_ERROR)
			{
				cerr << "ERROR: Failed to send versionNum\n";
				closesocket(listenSocket);
				cleanup(acceptSocket);
				return 1;
			}
		}
		else // If something was found wrong with the version number
		{
			cout << endl;
			cout << "versionNum: INVALID" << endl;
			
			// Send whether the version number was valid or invalid to the client
			int version = send(acceptSocket, invalidStatus, strlen(invalidStatus), 0);
